...
// --- 당직보고서 엑셀 다운로드 버튼 영구 추가 코드 (합계 계산 포함) ---
window.addEventListener('load', function() {
    const btn = document.createElement("button");
    btn.innerHTML = "📥 엑셀 보고서 다운로드";
    
    // 버튼 디자인 (우측 하단에 고정)
    btn.style.cssText = "position: fixed; bottom: 20px; right: 20px; z-index: 9999; padding: 12px 20px; background-color: #2e7d32; color: white; border: none; border-radius: 8px; font-weight: bold; font-size: 16px; cursor: pointer; box-shadow: 0 4px 6px rgba(0,0,0,0.3); transition: background 0.3s;";
    
    // 마우스 올렸을 때 색깔 살짝 변하게
    btn.onmouseover = function() { this.style.backgroundColor = "#1b5e20"; };
    btn.onmouseout = function() { this.style.backgroundColor = "#2e7d32"; };

    btn.onclick = function() {
        const year = prompt("다운로드할 '연도'를 입력하세요 (예: 2026)", new Date().getFullYear());
        if (!year) return;
        const month = prompt("다운로드할 '월'을 숫자만 입력하세요 (예: 3)");
        if (!month) return;

        const targetMonth = String(month).padStart(2, '0');
        const report = {};
        const names = ["김종환", "이승남", "이동현", "송우석", "김현석", "지은열", "송진우"];
        
        // 표 양식 세팅
        names.forEach(n => report[n] = {
            '월(3)': [], '화-목(4.5)': [], '금(8)': [], '토(16.5)': [], '일/공휴일(14.5)': [],
            '토eve(6)': [], '휴일day(12)': []
        });

        const getDayType = (y, m, d) => {
            const date = new Date(`${y}-${m}-${String(d).padStart(2, '0')}`);
            const wd = date.getDay(); 
            // 3.1절 예외 처리 (매년 3월 1일은 공휴일)
            if (m === '03' && d === 1) return '일/공휴일';
            if (wd === 0) return '일/공휴일'; 
            if (wd === 1) return '월';
            if (wd >= 2 && wd <= 4) return '화-목';
            if (wd === 5) return '금';
            if (wd === 6) return '토';
            return '일/공휴일';
        };
        ...
        if (!window.schedule) {
            alert("달력 데이터가 아직 로드되지 않았습니다. 잠시 후 다시 시도해주세요.");
            return;
        }
        ...
        for (let i = 1; i <= daysInMonth; i++) {
            const dateKey = `${year}-${targetMonth}-${String(i).padStart(2, '0')}`;
            const dayData = window.schedule[dateKey];
            if (!dayData) continue;
            
            // 야간이나 이브닝 데이터가 하나라도 있는지 확인
            if (dayData.night || dayData.evening) hasData = true;

            const dayType = getDayType(year, targetMonth, i);

            if (dayData.night) {
                dayData.night.split(' ').filter(Boolean).forEach(w => {
                    if (report[w]) {
                        if (dayType === '월') report[w]['월(3)'].push(i);
                        else if (dayType === '화-목') report[w]['화-목(4.5)'].push(i);
                        else if (dayType === '금') report[w]['금(8)'].push(i);
                        else if (dayType === '토') report[w]['토(16.5)'].push(i);
                        else report[w]['일/공휴일(14.5)'].push(i);
                    }
                });
            }
            
            if (dayData.evening) {
                dayData.evening.split(' ').filter(Boolean).forEach(w => {
                    if (report[w]) {
                        if (dayType === '토') report[w]['토eve(6)'].push(i);
                        else report[w]['휴일day(12)'].push(i);
                    }
                });
            }
        }
        ...
        if (!hasData) {
            alert(`${year}년 ${month}월에 입력된 당직 데이터가 없습니다. 달력 화면을 ${month}월로 넘긴 뒤 다시 버튼을 눌러보세요!`);
            return;
        }

        // 엑셀 첫 줄(헤더)에 '당직 합계' 칸 추가
        let csvContent = "\uFEFF성명,월(3),화-목(4.5),금(8),토(16.5),일·공휴일(14.5),토eve(6),휴일day(12),당직 합계\n";
        
        names.forEach(n => {
            const r = report[n];
            
            // 🔥 각 요일별 횟수에 배수(시간)를 곱해서 총합 계산
            const totalHours = 
                (r['월(3)'].length * 3) + 
                (r['화-목(4.5)'].length * 4.5) + 
                (r['금(8)'].length * 8) + 
                (r['토(16.5)'].length * 16.5) + 
                (r['일/공휴일(14.5)'].length * 14.5) + 
                (r['토eve(6)'].length * 6) + 
                (r['휴일day(12)'].length * 12);

            // 엑셀 마지막 칸에 합계(totalHours) 추가
            csvContent += `${n},"${r['월(3)'].join(', ')}","${r['화-목(4.5)'].join(', ')}","${r['금(8)'].join(', ')}","${r['토(16.5)'].join(', ')}","${r['일/공휴일(14.5)'].join(', ')}","${r['토eve(6)'].join(', ')}","${r['휴일day(12)'].join(', ')}",${totalHours}\n`;
        });
        ...
        link.href = url;
        link.download = `${year}년_${month}월_당직연장보고서.csv`;
